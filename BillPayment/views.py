...
# _____________ Get Bill Payments by Customer ID _____________

@api_view(["GET"])
def get_bill_payments_by_customer(request):
    customer_id = request.query_params.get('customer_id')
    if not customer_id:
        return Response({"detail": "Customer ID parameter is required"}, status=status.HTTP_400_BAD_REQUEST)

    bill_payments = BillPayment.objects.filter(customer_id=customer_id)
    if bill_payments.exists():
        serializer = BillPaymentSerializer(bill_payments, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    return Response({"detail": "No bill payments found for the specified customer"}, status=status.HTTP_404_NOT_FOUND)
